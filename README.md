...
- **Left bar** — *Shapes*: Box (B) · Cylinder · Sphere · Ramp · Wall. *Openings*: Opening (O) · Door · Light. *Outside*: Path (H) · Fence (N) · Tree · Tree brush · Terrain (T). *Play*: Spawn (S). Select is R. Every button has a tooltip.
- **Placing**: pick a shape, click the ground or any surface (stacking on other shapes works). `,` / `.` turn the preview 15° (with Shift, 90°). Shift+click snaps to a 0.5 m grid. Esc back to Select.
