...
- **Worlds**: `worlds/prison.json` is the full compound built from the aerial reference — the **PRISON** button loads it, or open `editor/?load=../worlds/prison.json`. It is generated by `node worlds/gen-prison.mjs` (every building's position and size is a line in that script, so tweak numbers there and regenerate). EXPORT downloads whatever is loaded as a `.json`.
