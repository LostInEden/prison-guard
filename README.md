...
- **Hollow (buildings)**: select one or more boxes / walls / cylinders / spheres and press **G** (or HOLLOW in the panel). They become a single *hollow* — a shell with walls, floor and roof of adjustable thickness (default 0.3 m) that you can walk into in play mode. Push overlapping pieces into each other by more than the wall thickness so their insides join into one space. Cylinders have a **Half** option (flat side on the origin, curve bulging out the back) for rounded ends. To cut a doorway or window: place a box poking through the wall, Shift+click the hollow so it's the primary selection, then **CARVE OPENINGS**. **SPLIT APART** returns the original pieces (cutters come back as red non-solid boxes). Hollows are stored as a recipe (parts + thickness + cuts) and rebuilt on load; the CSG comes from `three-bvh-csg` on the CDN.
- **Doors on hollows**: with the Door tool, click a hollow's wall — the door sets itself flush into the wall on the floor, the doorway is cut automatically, and the door swings inward. Free-standing doors swing *away from whoever opens them*; each door has a Swing setting (away / always in / always out). Moving an attached door doesn't move its doorway — Shift+click the hollow and CARVE again (CARVE accepts doors too).
